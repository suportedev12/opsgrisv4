...
import { useChecklistKPIs } from '@/hooks/useKPIs';
import { FilterBar } from '@/components/FilterBar';
import { KpiCard } from '@/components/KpiCard';
import { Panel } from '@/components/Panel';
import { BarChart, DonutChart } from '@/components/Charts';
import { Truck, CheckCircle2, AlertTriangle, Clock, Gauge, CalendarClock, Plus, Pencil, Trash2, Save, X } from 'lucide-react';
...
const EMPTY: Omit<ChecklistOperacional, 'id' | 'created_at' | 'updated_at'> = {
...
export function ChecklistView({ filters, onFiltersChange }: { filters: Filters; onFiltersChange: (f: Filters) => void }) {
  ...
  const kpis = useChecklistKPIs(filtered);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <KpiCard title="Total" value={kpis.total} icon={<Truck className="h-5 w-5" />} accent="orange" />
        ...
        <KpiCard title="Vencidos" value={kpis.vencidos} icon={<CalendarClock className="h-5 w-5" />} accent="red" />
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800/60 text-left text-xs uppercase tracking-wider text-slate-500">
                ...
                <th className="pb-2 pr-3 font-medium">Protocolo</th>
                ...
                <th className="pb-2 pr-3 font-medium">2º Motorista</th>
            ...
            <tbody>
              {filtered.map(r => {
                ...
                return (
                  <tr key={r.id} className={`border-b border-slate-800/40 transition-colors hover:bg-slate-800/30 ${isVencido ? 'bg-red-500/5' : ''}`}>
                    ...
                    <td className="py-2 pr-3 text-white">{r.protocolo ?? '-'}</td>
                    <td className="py-2 pr-3 text-slate-300">{r.eta ?? '-'}</td>
                    <td className="py-2 pr-3 text-slate-300">{r.origem ?? '-'}</td>
                    <td className="py-2 pr-3 text-white">{r.motorista ?? '-'}</td>
                    <td className="py-2 pr-3 text-slate-300">{r.telefone ?? '-'}</td>
                    <td className="py-2 pr-3 text-slate-300">{r.motorista_2 ?? '-'}</td>
                    ...
                    <td className={`py-2 pr-3 ${isVencido ? 'text-red-400 font-medium' : 'text-slate-300'}`}>{r.vencimento_checklist ? new Date(r.vencimento_checklist + 'T00:00').toLocaleDateString('pt-BR') : '-'}</td>
                    <td className="py-2 pr-3 text-slate-300">{r.horario_fim ?? '-'}</td>
                    <td className="py-2 pr-3 text-slate-300">{r.semana ?? '-'}</td>
              ...
              {filtered.length === 0 && <tr><td colSpan={24} className="py-8 text-center text-slate-500">Nenhum registro encontrado</td></tr>}
            </tbody>
          </table>
        </div>
      </Panel>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl border border-slate-700 bg-[#0f1117] p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-white">{editing ? 'Editar Checklist' : 'Novo Checklist'}</h2>
